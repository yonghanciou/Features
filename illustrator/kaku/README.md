...
Illustrator 的路徑量化特效外掛：把路徑對齊網格變成方塊像素，或是把曲線拆解成多邊形切面，兩種模式都能即時預覽。
...
- **多邊形**：把每一段曲線拆解成幾條直線切面，原本就是直線的段落維持不變，做出低多邊形（low-poly）的稜角感。
...
**不想裝外掛**：`src/Kaku.jsx` 是免安裝的獨立版本：Illustrator 的「檔案 > 指令碼 > Kaku.jsx」就能直接對選取路徑套用效果。這是較早期的邏輯，提供的模式跟參數跟原生版不完全一樣（見下方已知問題），是破壞性操作，也不會出現在外觀面板裡回頭調整，只是輕量替代方案。
...
3. 在跳出的對話框裡調整：
   - **模式**：像素化 / 多邊形。
   - **格數**（像素化用）：滑桿 + 數字欄位，沿選取範圍長邊要切成幾格。
   - **取樣密度**（像素化用）：1–20，愈大取樣愈細、愈貼合原本曲線，但也愈慢。
   - **面數**（多邊形用）：1–6，每段曲線要拆成幾條直線。
   - **預覽**：勾選後拖曳滑桿畫布會即時更新。
...
- 獨立 jsx 版本（`src/Kaku.jsx`）功能較舊：提供的是像素化與邊緣吸附兩種模式，沒有多邊形模式，複合路徑鏤空/斜角瑕疵也還沒修。

## 更新紀錄

- v1.0 — 初版：像素化與多邊形兩種模式、原生 Cocoa 對話框、即時預覽。
